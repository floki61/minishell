...
/*   Updated: 2022/08/20 04:23:59 by oel-berh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	export_error(char **str2, char *str, int error)
{
	if (error == 2)
...
int	check_exp(char *str)
{
	...
	str2 = 	NULL;
	if (!ft_strcmp(str, "="))
		return (export_error(str2, str, 1));
	if (str[0] == '#')
		return (2);
	else if (str[0] == '=')
		return (export_error(str2, str, 1));
	...
	if (pos == 1)
		...
		return (export_error(str2, skip_c(str, '!'), 2));
	if (!str2[1])
	{
		free_tab(str2, 0);
		return (-1);
